...
def run_smoke() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.screenshot(path=str(SCREENSHOT_DIR / "tauri-01-save-applied.png"), full_page=True)
            browser.close()
        finally:
        ...
        try:
            ...
            expect(page.get_by_text("Rolling Dice Native / Rolling Dice Native Motion")).to_be_visible()
            expect(page.get_by_text("applied / manual")).to_be_visible()
            expect(page.get_by_test_id("selected-part-inspector")).to_contain_text("1.08")
            ...
            expect(page.get_by_test_id("selected-part-inspector")).to_contain_text("1.08")
            page.screenshot(path=str(SCREENSHOT_DIR / "tauri-03-undo-redo.png"), full_page=True)
            browser.close()
        finally:
            stop_process_tree(process)
